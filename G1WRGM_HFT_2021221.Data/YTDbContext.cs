...
namespace G1WRGM_HFT_2021221.Data
{
    ...
    public class YTDbContext : DbContext
    {
        ...
        protected override void OnModelCreating(ModelBuilder modelBuilder)
        {
            ...
            {
                entity.HasMany(media => media.Videos).WithOne(user => user.YTContentCreator);
            });
            //One-Many Foreign Key reliationship between Users and Posts
            modelBuilder.Entity<Video>(entity =>
            {
                entity.HasMany(users => users.Comments).WithOne(post => post.Video);
            });
            //SocialMedia Database default rows
            modelBuilder.Entity<YTContentCreator>().HasData(new YTContentCreator { CreatorName = "Zsdav", Creation = 2004 });
